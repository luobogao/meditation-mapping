...
import { uploadCSV, addRecording, user, recordings, setCurrentRecording } from "./database";
    ...
    if (recordings.map(recording => recording.filename).includes(filename)) {
        ...
        recordings.forEach(recording =>
            {
                ...
                {
                    dataJSON.metadata = recording
                    dataJSON.id = recording.id
        ...
        menu.append("div").style("margin-top", "30px")
            ...
            .on("click", function () {
                ...
                var newMetadata = { filename: filename, user: o, notes: n, timestamp: firstRowTimestamp, startSecond: 0 }
                

                addRecording(newMetadata)
                    .then((doc) => {
                        console.log("----> Added recording: " + doc.id)
                                                
                        newMetadata.id = doc.id
                        dataJSON.id = doc.id
                        dataJSON.timestamp = firstRowTimestamp
                        dataJSON.metadata = newMetadata
                        addOrReplaceSession(dataJSON, function()
                        {
                            validate(dataJSON)
                        })
