...
function uploadNewCSV(dataJSON) {
    ...
    if (recordings.map(recording => recording.filename).includes(filename)) {
        ...
        updateRecording(recording).then(() => 
        {
                console.log("-----> Updated Firebase entry")
        ...
        uploadCSV(fileFullString, "MeditationRecordings", filename, metadata)
        ...
        menu.append("div").style("margin-top", "30px")
            ...
            .on("click", function () {
                ...
                var newRecord = { filename: filename, user: o, notes: n, timestamp: firstRowTimestamp, startSecond: 0, updatedTime: millis }
